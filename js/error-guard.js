...
(function (global) {
  ...
  function fingerprint(kind, detail) {
    return [
      kind,
      detail.message || detail.reason || '',
      detail.source || detail.filename || '',
      detail.line || detail.lineno || 0,
    ].join('\0');
  }
  ...
  function report(kind, detail) {
    ...
    global.LY_errors = global.LY_errors || [];
    global.LY_errors.push({
      kind: kind,
      at: Date.now(),
      message: detail.message || detail.reason || '',
      source: detail.source || detail.filename || '',
      line: detail.line || detail.lineno || 0,
  ...
  global.addEventListener('unhandledrejection', function (ev) {
    ...
    report('rejection', {
      ...
      source: reason && reason.fileName,
      line: reason && reason.lineNumber,
    });
    if (ev && ev.preventDefault) ev.preventDefault();
  });

  global.LY_safe = function (fn, fallback) {
    if (typeof fn !== 'function') return fn;
    return function lySafeWrapper() {
      try {
        return fn.apply(this, arguments);
      } catch (e) {
        report('caught', { message: e && e.message });
  ...
  function wrapGtag() {
    if (global._lyGtagWrapped || typeof global.gtag !== 'function') return;
    global._lyGtagWrapped = true;
    var orig = global.gtag;
    global.gtag = function () {
      try {
        return orig.apply(global, arguments);
      } catch (e) {
        report('gtag', { message: e && e.message });
      }
    };
  }

  function wrapConversionFns() {
    ['gtag_report_conversion', 'gtag_report_conversion_form'].forEach(function (name) {
      var fn = global[name];
      if (typeof fn !== 'function' || fn._lySafe) return;
      global[name] = global.LY_safe(fn, function () {
        return false;
      });
      global[name]._lySafe = true;
    });
  }
