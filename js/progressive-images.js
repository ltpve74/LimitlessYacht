...
(function (g) {
  ...
  g.LY_requestSharpUpgrade = function (el, priority) {
    ...
    if (priority === 'urgent') {
      ...
      if (g.LY_loadPreloadImage) {
        wrap.classList.add('ly-prog-sharp-loading');
        g.LY_loadPreloadImage(url, function () { revealSharp(wrap); });
      } else {
        revealSharp(wrap);
      }
      return;
  ...
  g.LY_revealProgressiveForUrl = function (url) {
    ...
    g.document.querySelectorAll('.ly-prog-wrap').forEach(function (wrap) {
      if (wrap.dataset.lySharpUrl === url && !wrap.classList.contains('ly-prog-sharp-ready')) {
        revealSharp(wrap);
      }
    });
  };
    ...
    if ('IntersectionObserver' in g) {
      var dwellIo = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          ...
          if (wrap.dataset.lyProgKind === 'hero') return;
      ...
      wraps.forEach(function (wrap) {
        if (wrap.dataset.lyProgKind !== 'hero') dwellIo.observe(wrap);
      });
    }

    wraps.forEach(function (wrap) {
      if (wrap.dataset.lyProgKind === 'hero') {
