...
- **Campaign promo banner** (`.hero-promo`, in the hero above the rates): a date-gated pill carrying `data-promo-until="YYYY-MM-DD"`. `lyInitRates()` hides any `.hero-promo` whose `data-promo-until` date has passed, so time-bound campaign copy (e.g. "Charter rates adjust from 1 July") self-removes — update or delete the banner + its locale PAIRS when the campaign changes. Translated in all three `i18n/locales/*.py`.
