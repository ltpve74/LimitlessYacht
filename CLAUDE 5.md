...
| `ly_charter_card_*`, `ly_cal_avail_*`, `ly_hero_rates_click`, `ly_hero_avail_click`, `ly_whatsapp_click`, `ly_email_click`, … | Various funnel / CTA interactions (grep `LY_clarityEvent(` in `index.html` for the full list). |
| `ly_cal_entry` (Clarity **tag**, not event) | First-touch calendar path, once per session, never overwritten. `cta` = hero Check available dates; `scroll` = `ly_section_view_availability` fired with no value set yet. |
...
**Reviews snippet + CLS reserve:** the visible snippet is clamped to **3 lines** (`.review-text--clamped{-webkit-line-clamp:3;min-height:3lh}`) — `min-height:3lh` makes every clamped card the same height. The reviews load lazily into `#reviewsGrid` (replacing `#reviewsLoading`), so to stop a layout shift the placeholder **and** the grid reserve the loaded height: `min-height:145rem` mobile / `70rem` desktop. **These reserves are sized for exactly 13 reviews — if you add/remove a review, re-measure and update both `min-height` values**, or the section will shift on load again (CLS). The `…more` expand still works (removing `.review-text--clamped` drops the clamp + reserve). Short reviews that already fit 3 lines do not show `…more`.
